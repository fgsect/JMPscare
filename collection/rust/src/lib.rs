...
use std::rc::Rc;
use std::rc::Weak;
use unicornafl::unicorn_const::{uc_error, Arch, Mode};
...
pub fn setup_cov_collection<D>(
    uc: &mut unicornafl::UnicornHandle<D>,
    code_start: u64,
    code_end: u64,
    cov_data: Rc<RefCell<Vec<u64>>>,
) {
    let callback = Box::new(
        move |uc: unicornafl::UnicornHandle<D>, addr: u64, size: u32| {
    ...
    uc.add_code_hook(code_start, code_end, callback)
        .expect("Failed to add coverage collection hook");
}
